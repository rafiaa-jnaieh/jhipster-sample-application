...
export class DashboardItemRoutingResolveService implements Resolve<IDashboardItem> {
  constructor(protected service: DashboardItemService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IDashboardItem> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((dashboardItem: HttpResponse<DashboardItem>) => {
